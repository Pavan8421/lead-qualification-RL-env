    ...
    * The loop here is intentionally thin: most of the heavy lifting lives in
    ...
    async def _collect_group(
        ...
        async def _one(g_idx: int) -> EpisodeRollout:
            async with sem:
                # IMPORTANT: each concurrent rollout needs its OWN WebSocket
                # connection. A single EnvClient cannot service concurrent
                # recv() calls ("cannot call recv while another coroutine
                # is already running recv") and once it errors the
                # connection is dead for the rest of training.
                env = LeadTriageEnv(base_url=self.config.env_base_url)
                try:
                    await env.connect()
                    # Vary seed within the group so the G rollouts actually
                    # differ in stochastic-outcome realisations (otherwise
    ...
    async def step_once(self, *, step_index: int) -> Dict[str, float]:
        ...
        # B prompts × G generations per optimizer step. Compute advantages
        # *within each group* (correct GRPO baseline), then accumulate across
        # groups for one combined gradient update.
        n_prompts = max(1, int(self.config.episodes_per_step))
        all_rollouts: List[EpisodeRollout] = []
        all_advantages: List[float] = []
        all_scalars: List[float] = []
        per_group_adv_std: List[float] = []
        per_group_action_diversity: List[float] = []

        for b in range(n_prompts):
            prompt_seed = self.config.seed + step_index * 1000 + b
            group = await self._collect_group(
                seed=prompt_seed,
                tier=tier,
            ...
            group_scalars = [r["scalar_reward"] for r in group_rewards]
            group_adv = group_advantages(group_scalars)

            all_rollouts.extend(group)
            all_scalars.extend(group_scalars)
            all_advantages.extend(group_adv)
            per_group_adv_std.append(_std(group_adv))

            # Action-diversity probe: fraction of unique action-token sequences
            # within this group. 1.0 = every rollout took a distinct path;
            # ~1/G = total mode collapse.
            seqs = tuple(
                tuple(s.action_token for s in ro.steps) for ro in group
            )
            uniq = len(set(seqs))
            per_group_action_diversity.append(uniq / max(1, len(seqs)))
        ...
        # Surface rollout errors so dead env servers / network issues don't
        # silently produce zero-reward steps for hours.
        n_errors = sum(1 for r in all_rollouts if r.error)
        if n_errors:
            sample_err = next((r.error for r in all_rollouts if r.error), "?")
            print(
                f"[ROLLOUT-ERR] step {step_index}: {n_errors}/{len(all_rollouts)} "
                f"rollouts errored. First error: {sample_err}",
                flush=True,
            )
        ...
        for rollout, adv in zip(all_rollouts, all_advantages):
        ...
        metrics: Dict[str, float] = {
            ...
            "n_rollouts": float(len(all_rollouts)),
            "mean_scalar_reward": float(sum(all_scalars) / max(1, len(all_scalars))),
            "mean_terminal_grader": float(sum(r["terminal_grader"] for r in rewards) / n),
            "mean_total_step_reward": float(sum(r["total_step_reward"] for r in rewards) / n),
            "conversion_rate": float(sum(r["converted"] for r in rewards) / n),
            "mean_invalid_resamples": float(
                sum(r["invalid_resamples_total"] for r in rewards) / n
            ),
            "max_repeat_streak": float(max((r["repeat_streak_max"] for r in rewards), default=0)),
            "advantage_std": float(_std(all_advantages)),
            "mean_within_group_adv_std": float(
                sum(per_group_adv_std) / max(1, len(per_group_adv_std))
            ),
            "mean_action_diversity": float(
                sum(per_group_action_diversity) / max(1, len(per_group_action_diversity))
            ),
